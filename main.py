...
def checkLigne(liste):
    '''regarde si un même symbole aparaît 4 fois d'affilé dans toutes les listes de la liste en paramètre (lignes du plateau)
       ...
       si il y a une victoire renvoie un tuple avec victoire et le joueur sinon renvoie False
    '''
    rep=(False,0)
...
def affichage(l):
    '''
       Permet d'aficher le plateau de jeu sans rien retourner
...
def pion(signe, l,IA):
    '''
       demande au joueur de jouer et modifie la liste en fonction sans rien retourner
       : paramètre: signe: 1 ou 2 selon le joueur qui joue
                  ...
                  : IA: si le joueur 2 est une IA ou non
    '''
    if IA==True:
        coupJoue=False
...
def jeu(IA):
    '''
       lance la partie jusqu'à ce que run ne soit plus égale à true donc jusqu'à ce que la partie se termine
       paramètre: IA: True si le joueur souhaite jouer avec une IA, False sinon
    '''
